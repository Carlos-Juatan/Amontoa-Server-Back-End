...
export default async function connectToDatabase(uri) { // Define uma função assíncrona chamada conectarAoBanco que recebe como parâmetro a string de conexão do banco de dados. A palavra-chave async indica que a função pode conter operações assíncronas, como a conexão com o banco de dados.
    ...
    try { // Inicia um bloco try...catch para tratar possíveis erros durante o processo de conexão.
        mongoClient = new MongoClient(uri); // Cria uma nova instância do MongoClient, passando a string de conexão como argumento. Essa instância representa a conexão com o banco de dados.
